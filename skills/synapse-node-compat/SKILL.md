...
Master cross-version Node.js engineering for Synapse. This skill ensures that the application remains stable from Node 18 through Node 23+, handling native dependency variations and API availability boundaries with precision.

## Core Concepts

### 1. Capability Gating
Instead of hard engine bumps, Synapse prefers gating specific features (like the SQLite-backed memory subsystem) based on the availability of built-in modules or specific method signatures.
...
- [Synapse Version Boundaries](./references/version-boundaries.md)
