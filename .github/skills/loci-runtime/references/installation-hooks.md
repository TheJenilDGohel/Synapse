...
Loci uses a multi-tier hook system to ensure skills are always up to date.
...
When `loci` is updated via npm, the post-install hook triggers `install-loci-skill.mjs` automatically unless `LOCI_SKIP_SKILL_INSTALL` is set.
