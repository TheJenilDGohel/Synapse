#!/usr/bin/env node
// Loci Pre-Tool Hook for Claude Code
//
...
//
// Triggers on: Edit, Write, Bash, MultiEdit, Read, Grep, mcp__loci__*
//
// How it works:
...
const DEBOUNCE_FILE = path.join(TMP_DIR, 'loci-pre-hook-last.json');
...
process.stdin.on('end', () => {
  ...
  try {
    ...
    const markerPath = path.join(TMP_DIR, `loci-prime-${sessionKey}.flag`);
    ...
    const result = spawnSync(process.execPath, [LOCI_CLI, 'memory', 'prime', '--json', '--', query], {
      encoding: 'utf8',
      timeout: 8000,
      shell: false,
...
function shouldShoutNow() {
  const shoutFile = path.join(TMP_DIR, 'loci-shout-last.json');
  try {
...
function buildShoutContext() {
  return [
    '',
    '================================================================',
    '[ACTION REQUIRED] Loci session is NOT primed.',
    '================================================================',
    '',
    'You MUST call mcp__loci__synapse_agent_prime BEFORE the next',
    ...
    '',
    'Do this now:',
    '  1. Call mcp__loci__synapse_agent_prime with a short task',
