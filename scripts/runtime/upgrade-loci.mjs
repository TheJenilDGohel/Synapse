...
import { migrateLociHomeLayout, resolveLociHome } from '../../src/core/runtime/index.ts';
...
import { NPM_BIN, LOCI_BIN, isWindows } from '../../src/core/runtime/platform.ts';
...
const lociHome = resolveLociHome({ HOME: process.env.HOME || '' });
...
async function main() {
  if (hasFlag('help') || hasShortFlag('h')) {
    console.log(c.bold('Loci upgrade helper'));
    console.log('');
    console.log(c.bold('Usage:'));
    console.log('  loci upgrade');
    console.log('  loci upgrade stable');
    console.log('  loci upgrade beta');
    console.log(`  loci upgrade ${c.cyan(SERVER_VERSION)}`);
    console.log(`  loci upgrade ${c.cyan('install')} ${c.cyan(SERVER_VERSION)}`);
    console.log('');
    console.log(c.bold('Options:'));
    console.log(`  ${c.cyan('--version')}=<semver|latest|stable|beta>  target package version`);
    console.log(`  ${c.cyan('--package')}=<npm-package>              package name (default loci)`);
  ...
  const rawPackageName = parseArg('package') || 'loci';
  ...
  const skillCmd = LOCI_BIN;
  const setupScript = path.resolve(scriptsDir, 'setup-loci.mjs');
  const setupArgs = [
    setupScript,
    `--package=${packageName}`,
  ...
  const doctorScript = path.resolve(scriptsDir, 'doctor-loci.mjs');
  try {
  ...
  console.log(box.row(c.bold('Loci Upgrade Completed ✓'), 60));
...
main().catch((error) => {
  process.stderr.write(`[loci-upgrade] fatal: ${error?.message || error}\n`);
  process.exit(1);
});
