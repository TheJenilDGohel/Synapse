...
function generateClientNativeFiles(targetSkillDir, toolFamily, skillContent) {
  ...
  switch (toolFamily) {
    case 'cursor': {
      ...
      const ruleContent = [
        '---',
        'description: Loci Expert — Persistent Memory, Code Search, and Knowledge Graph SOP',
        'globs:',
        '  - "**/*"',
        'alwaysApply: true',
        '---',
      ...
      fs.writeFileSync(path.join(rulesDir, 'loci.mdc'), ruleContent, 'utf8');
      break;
    }
    case 'windsurf': {
      const rulesDir = path.join(grandparentDir, 'rules');
      ...
      fs.writeFileSync(path.join(rulesDir, 'loci.md'), wsContent, 'utf8');
      break;
    }
    case 'opencode': {
      const agentsContent = [
        '---',
        'description: Loci Expert — Persistent Memory, Code Search, and Knowledge Graph SOP',
        'keywords: [loci, memory, knowledge, graph, search, code, file, project, nest, branch, recall, ingest, diary, triple, entity]',
        'globs: ["**/*"]',
        'match: any',
        '---',
        '',
    ...
    case 'cline': {
      ...
      fs.writeFileSync(path.join(rulesDir, 'loci.md'), clineContent, 'utf8');
      break;
    }
    case 'continue': {
      const rulesDir = path.join(grandparentDir, 'rules');
      ...
      fs.writeFileSync(path.join(rulesDir, 'loci.md'), continueContent, 'utf8');
      break;
    }
    default:
      break;
...
export function main() {
  ...
  if (!quiet) {
    console.log(`\n${C.bold}${C.blue}Loci Skill Installer${C.reset}`);
