...
} from '../../src/core/setup/client-installer.ts';
import ora from 'ora';
import {
  c,
  symbol,
...
const lociHome = resolveLociHome(homeOnlyEnv);
...
function runPreflightChecks() {
  ...
  if (Number.isFinite(majorNode) && majorNode < 22) {
    warnings.push(`Node.js ${process.versions.node} does not provide built-in node:sqlite. Loci setup will use the json index backend and disable local memory unless you upgrade to Node 22+.`);
  }

  const hasGlobal = commandExists(getGlobalCommand());
  const hasNpx = commandExists(getNpxCommand());
  if (!hasGlobal && !hasNpx) {
    errors.push('Neither loci nor npx is available. Install Loci globally or fix Node.js/npm and retry.');
  }
...
function buildLociEnv(indexConfig) {
  const env = {
    MCP_MODE: 'stdio',
    LOCI_CONFIG: configPath,
    LOCI_INDEX_BACKEND: indexConfig.backend,
    LOCI_DB_PATH: indexConfig.dbPath,
    LOCI_INDEX_PATH: indexConfig.indexPath,
    LOCI_EMBED_PROVIDER: indexConfig.embedding.provider,
    LOCI_EMBED_MODEL: indexConfig.embedding.model,
    LOCI_EMBED_CACHE_DIR: indexConfig.embedding.cacheDir,
    LOCI_EMBED_DIMS: String(indexConfig.embedding.dimensions),
    LOCI_RERANKER_PROVIDER: indexConfig.reranker.provider,
    LOCI_RERANKER_MODEL: indexConfig.reranker.model,
    LOCI_RERANKER_CACHE_DIR: indexConfig.reranker.cacheDir,
    LOCI_MEMORY_ENABLED: String(indexConfig.memory.enabled),
    LOCI_MEMORY_BACKEND: indexConfig.memory.backend,
    LOCI_MEMORY_DB_PATH: indexConfig.memory.dbPath
  };
  if (indexConfig.sqliteVecExtensionPath) {
    env.LOCI_SQLITE_VEC_EXTENSION = indexConfig.sqliteVecExtensionPath;
  }
  return env;
}
...
function buildClientSnippet(packageRef, indexConfig) {
  ...
  return {
    mcpServers: {
      loci: buildLociServerConfig({
        command: launch.command,
        args: launch.args,
        env: buildLociEnv(indexConfig)
      })
    }
  };
}
...
function resolveSqliteVecPreference(indexConfig) {
  ...
  const installResult = ensureSqliteVecExtension({
    lociHome,
    env: { ...homeOnlyEnv, LOCI_SQLITE_VEC_SEARCH_DIRS: process.env.LOCI_SQLITE_VEC_SEARCH_DIRS || '' },
...
function installClientConfigs(packageRef, indexConfig) {
  ...
  const serverConfig = snippet.mcpServers.loci;
  return installLociIntoDetectedClients({
    serverConfig,
...
async function warmupModels(indexConfig) {
  ...
  if (indexConfig?.embedding?.provider && indexConfig.embedding.provider !== 'none') {
    ...
    try {
      ...
      await embedding.embed('loci embedding warmup');
...
function printSuccess(packageRef, indexConfig) {
  ...
  console.log(box.row(c.bold('Loci Setup Complete ✓'), 60));
  ...
  console.log('');
  console.log('Try: loci doctor');
  console.log(rule());
}

async function main() {
  if (argv.includes('--help') || argv.includes('-h')) {
    console.log('Loci setup');
    console.log('');
    console.log('Usage:');
    console.log('  loci setup');
    console.log('  loci setup --paths="/abs/path1,/abs/path2"');
    console.log('  loci setup --roots-json=\'[{"label":"repo","path":"/abs/repo"}]\'');
    console.log('  loci setup --package="loci"');
    console.log('  loci setup --skip-model-download=true');
    console.log('  loci setup --skip-sqlite-vec-install=true');
    console.log('  loci setup --sqlite-vec-extension="/abs/path/to/vec0.so"');
    console.log('  loci setup --index-backend=sqlite-vec');
    console.log('  loci setup --memory-db-path="/abs/path/to/memory.db"');
    console.log('');
  ...
  const envPkg = process.env.LOCI_NPX_PACKAGE;
  const packageRef = parseArg('package') || (typeof envPkg === 'string' && envPkg.length > 0 ? envPkg.replace(/[^a-zA-Z0-9@/._-]/g, '') : 'loci');
...
main().catch((error) => {
  console.error('[loci-setup] fatal:', error?.message || error);
  process.exit(1);
});
