...
import { installRuntimeWarningFilter } from '../../src/core/runtime/index.ts';
...
async function runMemorySuite(tempRoot) {
  const { MemoryStore } = await import('../../src/core/engine/index.ts');
...
async function main() {
  const {
    ...
    AstChunker,
    RerankerService
  } = await import('../../src/core/engine/index.ts');
  const { SqliteVecIndexService } = await import('../../src/core/engine/retrieval/sqlite-vec/service.ts');
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'synapse-stress-'));
  const project = path.join(root, 'app');
  fs.mkdirSync(project, { recursive: true });
  const useEmbeddings = String(process.env.SYNAPSE_STRESS_EMBED || '').toLowerCase() === 'true';
  const useReranker = String(process.env.SYNAPSE_STRESS_RERANKER || '').toLowerCase() === 'true';
  const repeats = Math.max(1, Number.parseInt(process.env.SYNAPSE_STRESS_REPEATS || '3', 10) || 3);
  const embeddingService = new EmbeddingService({
    provider: useEmbeddings ? 'huggingface' : 'none',
    model: process.env.SYNAPSE_STRESS_EMBED_MODEL || 'sentence-transformers/all-MiniLM-L6-v2',
    cacheDir: process.env.SYNAPSE_STRESS_EMBED_CACHE_DIR || path.join(root, '.cache')
  });
  const astChunker = new AstChunker();
  const reranker = new RerankerService({
    provider: useReranker ? 'huggingface' : 'none',
    model: process.env.SYNAPSE_STRESS_RERANKER_MODEL || 'cross-encoder/ms-marco-MiniLM-L-6-v2',
    cacheDir: process.env.SYNAPSE_STRESS_RERANKER_CACHE_DIR || path.join(root, '.cache')
  });
  ...
  try {
    ...
    try {
      ...
      console.error('[stress-synapse] warning: failed to remove temp directory:', error?.message || error);
    }
  }
}

main().catch((error) => {
  console.error('[stress-synapse] fatal:', error?.message || error);
  process.exit(1);
});
