...
/**
 * Loci unified CLI entry point.
 *
 * Supports noun-verb subcommands (loci memory add, loci kg query, ...)
 * and legacy flat commands (loci setup, loci doctor, ...).
 *
...
async function main() {
  ...
  // `loci start` / `loci serve`
  if (command === 'start' || command === 'serve') {
    if (rest.includes('--help') || rest.includes('-h')) {
      process.stdout.write('Loci MCP server\n\n');
      process.stdout.write('Usage:\n');
      process.stdout.write('  loci start\n');
      process.stdout.write('  loci serve\n');
  ...
  // `loci install skills` (backward compat alias)
  if (command === 'install' && rest[0] === 'skills') {
    process.argv = buildForwardArgv(rest.slice(1), process.argv);
    const mod = await importRelative('../scripts/runtime/install-loci-skill.mjs', import.meta.url);
    await mod.main();
    return;
  }
...
main().catch((error) => {
  process.stderr.write(`[loci-cli] fatal: ${error?.message || error}\n`);
  process.exit(1);
});
