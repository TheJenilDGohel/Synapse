...
const { toMinimalWriteResponse, stripEmptyFields } = await import('../src/interfaces/mcp/common/terse-utils.js');
...
test('MCP registration: all 13 write tools include terse parameter', async () => {
  const { registerMemoryStoreTools } = await import('../src/interfaces/mcp/tools/memory-store.js');
  const { registerMemoryWorkflowTools } = await import('../src/interfaces/mcp/tools/memory-workflow.js');
  const { registerGraphTools } = await import('../src/interfaces/mcp/tools/graph-tools.js');
  const {
  ...
  } = await import('../src/interfaces/mcp/common/index.js');
  ...
  const writeToolNames = [
    'synapse_memory_store', 'synapse_memory_update', 'synapse_memory_delete',
    'synapse_memory_capture_event', 'synapse_memory_add_relation', 'synapse_memory_remove_relation',
    'synapse_capture_outcome',
    'synapse_kg_add_entity', 'synapse_kg_add_triple', 'synapse_kg_invalidate',
    'synapse_diary_write', 'synapse_ingest_markdown', 'synapse_ingest_json'
  ];
