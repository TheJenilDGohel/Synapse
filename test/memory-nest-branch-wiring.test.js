...
test('manageBranches supports merge, delete, and stale listing', async (t) => {
  ...
  const oldA = await store.storeEntry({
    ...
    scope: { project_path: '/tmp/localnest' },
    nest: 'localnest',
  ...
  const branchesAfterMerge = await store.listBranches('localnest');
  ...
  const cleared = await store.listEntries({ projectPath: '/tmp/localnest', nest: 'localnest' });
