...
import { MemoryWorkflowService } from '../src/core/engine/memory.js';
...
test('getTaskContext recalls memory when enabled and query is present', async () => {
  const service = new MemoryWorkflowService({
    ...
    getRuntimeSummary: async () => ({ name: 'synapse', version: 'test' })
  });
  ...
  assert.equal(out.runtime.name, 'synapse');
});
...
test('captureOutcome accepts zero args as a session-end marker', async () => {
  let received = null;
  const memory = createMemoryStub();
  memory.captureEvent = async (input) => {
    received = input;
  ...
  assert.equal(out.captured, true);
  assert.equal(received.event_type, 'task');
  assert.equal(received.title, 'Session completed');
  assert.match(received.summary, /Session completed with 3 memories and 7 recorded events/);
});
