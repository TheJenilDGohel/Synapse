---
description: Beta release notes for synapse-mcp 0.0.7-beta.2 covering temporal knowledge graph, multi-hop traversal, nest/branch hierarchy, agent-scoped memory, semantic dedup, conversation ingestion, hooks system, CLI-first architecture, and 17 new MCP tools.
---

# 0.0.7-beta.2

Beta release notes for `synapse-mcp@0.0.7-beta.2`.
...
- Unified noun-verb CLI: `synapse <noun> <verb>` subcommand pattern with zero new dependencies.
...
- Legacy deprecation: old `synapse-mcp-*` binaries now show deprecation warnings and redirect to canonical commands.
...
| Category | Tools |
| --- | --- |
| Knowledge Graph (7) | `synapse_kg_add_entity`, `synapse_kg_add_triple`, `synapse_kg_query`, `synapse_kg_invalidate`, `synapse_kg_as_of`, `synapse_kg_timeline`, `synapse_kg_stats` |
| Nest/Branch (3) | `synapse_nest_list`, `synapse_nest_branches`, `synapse_nest_tree` |
| Graph Traversal (2) | `synapse_graph_traverse`, `synapse_graph_bridges` |
| Agent Diary (2) | `synapse_diary_write`, `synapse_diary_read` |
| Conversation Ingestion (2) | `synapse_ingest_markdown`, `synapse_ingest_json` |
| Duplicate Check (1) | `synapse_memory_check_duplicate` |
| Hook Introspection (2) | `synapse_hooks_stats`, `synapse_hooks_list_events` |
...
This beta transforms Synapse from a code retrieval + memory server into a full knowledge management platform. The temporal KG enables structured fact tracking with time-travel queries. Multi-hop traversal lets agents discover indirect connections no other local tool can find. The CLI-first architecture means every MCP capability is also usable directly from the terminal.

## When to Use This Page

- testing `synapse-mcp@0.0.7-beta.2`
