...
export function normalizeMemorySuggestionResult(result: any, id: string, threshold: number, options: { includeLegacyArrays?: boolean } = {}): NormalizedMemorySuggestionResult {
  const suggestions = Array.isArray(result?.suggestions) ? result.suggestions : [];
  const base = { ...(result || {}) };
  delete base.suggestions;
  delete base.items;
  ...
  const normalized: NormalizedMemorySuggestionResult = {
    ...base,
    ...paginated,
    id: result?.id ?? id,
  ...
  if (options.includeLegacyArrays) normalized.suggestions = suggestions;
  return normalized;
}
