...
export function normalizeEmbedStatus(status: Record<string, unknown> | null | undefined): NormalizedEmbedStatus {
  ...
  const sqlite_vec_extension = (status?.sqlite_vec_extension as Record<string, unknown>) || null;
  ...
  return {
    ...
    sqlite_vec_loaded: (status?.sqlite_vec_loaded as boolean) ?? (sqlite_vec_extension?.loaded as boolean) ?? null,
...
export function normalizeTaskContextResult(result: Record<string, unknown> | null | undefined, input: Record<string, unknown> | null | undefined = {}): NormalizedTaskContextResult {
  ...
  return {
    query: (result?.query as string) || (inputObj.query as string) || (inputObj.task as string) || '',
    scope: {
      root_path: (scope.root_path as string) || (inputObj.root_path as string) || '',
      project_path: (scope.project_path as string) || (inputObj.project_path as string) || '',
      branch_name: (scope.branch_name as string) || (inputObj.branch_name as string) || '',
      topic: (scope.topic as string) || (inputObj.topic as string) || '',
      feature: (scope.feature as string) || (inputObj.feature as string) || ''
    },
