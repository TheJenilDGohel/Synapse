  ...
  if (runtime.indexBackend === 'sqlite-vec') {
    try {
      ...
      return new SqliteVecIndexService({
        ...
        sqliteVecModule: runtime.sqliteVecModule,
...
export async function createServices(runtime: any): Promise<AppServices> {
  ...
  let enrichmentService: any = null;
  if (runtime.enrichmentEnabled) {
    const { EnrichmentService } = await import('../../core/engine/enrichment/service.js');
    enrichmentService = new EnrichmentService({
      device: runtime.enrichmentDevice,
      modelId: runtime.enrichmentModel,
      cacheDir: runtime.embeddingCacheDir
    });
  }
  ...
  // Inject enrichmentService into vectorIndex if it's a SqliteVecIndexService
  ...
  // Initialize on-device NER for entity extraction (opt-in, issue #67)
  if (runtime.nerEnabled) {
    const { configureNER } = await import('../../core/engine/memory/ingest/entity-extractor.js');
    configureNER(true, {
      model: runtime.nerModel,
      cacheDir: runtime.embeddingCacheDir,
      confidenceThreshold: runtime.nerConfidenceThreshold
    });
  }

  // Initialize on-device memory classifier (opt-in, issue #68)
  if (runtime.classifierEnabled) {
    const { configureClassifier } = await import('../../core/engine/memory/ingest/ingest.js');
    configureClassifier(true, {
      model: runtime.classifierModel,
      cacheDir: runtime.embeddingCacheDir,
      confidenceThreshold: runtime.classifierConfidenceThreshold
    });
  }
