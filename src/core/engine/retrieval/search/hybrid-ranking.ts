  ...
  return rows
    ...
    .map((item) => {
      ...
      if (item.enrichment) {
        ...
        if (item.enrichment.intent?.toLowerCase().includes(query)) {
          metadataScore += 0.05;
        }
        if (item.enrichment.tags?.some(t => query.includes(t.toLowerCase()))) {
          metadataScore += 0.03;
        }
      }

      let rrfScore = item.lexical_score + item.semantic_score + metadataScore;
