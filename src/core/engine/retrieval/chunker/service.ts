...
export class AstChunker {
  ...
  queryCache: Map<string, any> = new Map();
  ...
  async getParser(languageId: string): Promise<{ parse: (text: string) => { rootNode: TreeSitterNode } } | null> {
    ...
    try {
      ...
      console.error(`Error setting language for ${languageId}:`, e);
      throw e;
    }
  ...
  async getQuery(languageId: string, language: any): Promise<any> {
    ...
    try {
      ...
      this.queryCache.set(languageId, q);
      return q;
    } catch (e) {
      console.warn(`Failed to load query for ${languageId}:`, e);
      this.queryCache.set(languageId, null);
      return null;
    }
  }

  collectCandidateNodes(root: TreeSitterNode, languageId: string, query?: any): TreeSitterNode[] {
    if (query) {
      return query.captures(root).map((c: any) => c.node);
    }
  ...
  async buildAstSlices({ text, chunkLines, chunkOverlap, languageId }: BuildAstInput): Promise<ChunkSlice[] | null> {
    try {
      ...
      const query = await this.getQuery(languageId, language);
      ...
      const candidates: CandidateNode[] = this.collectCandidateNodes(root, languageId, query)
        .map((node) => {
      ...
      console.error('buildAstSlices error:', e);
      this.fallbackLanguages.add(languageId);
      return null;
    }
  }
