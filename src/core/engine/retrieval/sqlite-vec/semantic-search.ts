...
export async function semanticSearch(
  ...
  state: SearchState,
  { query, projectPath, allRoots, maxResults, minScore, probes }: SemanticSearchOptions
  ...
  let vecRows: { file_path: string; start_line: number; end_line: number; preview: string; distance: number; enrichment_json?: string }[] | null = null;
  if (queryEmbedding && sqliteVecTableReady) {
    try {
      const k = Math.max(maxResults * 6, 64);
      const probesClause = probes ? ` AND probes = ${probes}` : '';
      vecRows = db.prepare(
        `SELECT c.file_path, c.start_line, c.end_line, c.preview, v.distance, c.enrichment_json
         FROM vec_chunks v
         JOIN chunks c ON c.rowid = v.chunk_rowid
         WHERE v.embedding MATCH ? AND k = ${k}${probesClause}
           AND (${baseScope.where})
  ...
  const rows: ChunkRow[] = Array.isArray(vecRows)
    ...
    : (queryEmbedding
      ? db.prepare(
        `SELECT c.file_path, c.start_line, c.end_line, c.preview, c.terms_json, c.term_count, c.embedding_json, c.enrichment_json
         FROM chunks c
      ...
      : (() => {
        ...
        return db.prepare(
          `SELECT DISTINCT c.file_path, c.start_line, c.end_line, c.preview, c.terms_json, c.term_count, c.embedding_json, c.enrichment_json
           FROM term_index ti
  ...
  for (const row of rows) {
    ...
    // Boost score based on enrichment metadata if available
    if (row.enrichment_json) {
      try {
        const enrichment = JSON.parse(row.enrichment_json);
        const lowerQuery = query.toLowerCase();

        // Check if query matches intent or tags
        if (enrichment.intent?.toLowerCase().includes(lowerQuery)) {
          score = Math.min(1.0, score + 0.1);
        }
        if (Array.isArray(enrichment.tags) && enrichment.tags.some((t: string) => lowerQuery.includes(t.toLowerCase()))) {
    ...
    if (row.enrichment_json) {
      try { enrichment = JSON.parse(row.enrichment_json); } catch { /* ignore */ }
    }
    out.push({
      file: row.file_path,
