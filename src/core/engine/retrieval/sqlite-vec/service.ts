...
interface EnrichmentServiceLike {
  enrich(id: string, code: string): Promise<any>;
...
export interface SqliteVecServiceOptions {
  ...
  enrichmentService?: EnrichmentServiceLike | null;
...
export class SqliteVecIndexService {
  ...
  constructor({
    ...
    this.sqliteVecModule = sqliteVecModule;
  ...
  ensureDb(): void {
    ...
    try {
      ...
      if (this.sqliteVecTableReady) {
        const vecCount = (this.db.prepare('SELECT COUNT(*) AS c FROM vec_chunks').get() as { c: number })?.c || 0;
        const chunkCount = (this.db.prepare('SELECT COUNT(*) AS c FROM chunks').get() as { c: number })?.c || 0;
        if (vecCount === 0 && chunkCount > 0) {
          this.syncSqliteVecRowsFromChunks();
        }
      }

      const dbDir = path.dirname(this.dbPath);
      const jsonPath = path.join(dbDir, 'index.json');
      const rowCount = (this.db.prepare('SELECT COUNT(*) AS c FROM chunks').get() as { c: number })?.c || 0;
      if (rowCount === 0 && fs.existsSync(jsonPath)) {
        try {
          const raw = fs.readFileSync(jsonPath, 'utf8');
          const data = JSON.parse(raw);
          if (data.chunks && Array.isArray(data.chunks)) {
            this.runInTransaction(() => {
              const stmtFile = this.db!.prepare('INSERT OR IGNORE INTO files(path, signature, updated_at) VALUES (?, ?, ?)') as unknown as DbStatement;
              const stmtChunk = this.db!.prepare('INSERT OR REPLACE INTO chunks(id, file_path, start_line, end_line, preview, terms_json, term_count, embedding_json, norm) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)') as unknown as DbStatement;
              const stmtTerm = this.db!.prepare('INSERT OR IGNORE INTO term_index(term, chunk_id) VALUES (?, ?)') as unknown as DbStatement;
              
              for (const chunk of data.chunks) {
                const fp = chunk.file_path || chunk.filePath || '';
                stmtFile.run(fp, '', null);
                
                const termsJson = Array.isArray(chunk.terms) ? JSON.stringify(chunk.terms) : '[]';
                const termCount = Array.isArray(chunk.terms) ? chunk.terms.length : 0;
                const embeddingJson = Array.isArray(chunk.embedding) ? JSON.stringify(chunk.embedding) : null;
                const chunkId = chunk.id || `${fp}:${chunk.start_line || 0}`;
                
                stmtChunk.run(
                  chunkId,
                  fp,
                  chunk.start_line || chunk.startLine || 0,
                  chunk.end_line || chunk.endLine || 0,
                  chunk.preview || '',
                  termsJson,
                  termCount,
                  embeddingJson,
                ...
                if (Array.isArray(chunk.terms)) {
                  for (const term of chunk.terms) {
                    stmtTerm.run(term, chunkId);
                  }
                }
              }
            });
            this.syncSqliteVecRowsFromChunks();
            fs.renameSync(jsonPath, path.join(dbDir, 'index.json.migrated'));
          }
        } catch (e) {
          console.warn('[sqlite-vec-index] failed to migrate legacy index.json:', e);
        }
      }
    } catch (error) {
      this.resetDb();
  ...
  async indexProject({ projectPath, allRoots, force, maxFiles, onProgress }: {
    ...
    for (const filePath of files) {
      try {
        ...
        const rawChunks = await chunkFileFn(filePath, text, {
        ...
        // Run enrichment in parallel for all chunks in the file
        const chunks = await Promise.all(rawChunks.map(async (chunk) => {
          let enrichment: any = null;
          if (this.enrichmentService) {
            try {
              enrichment = await this.enrichmentService.enrich(chunk.id, chunk.raw_text);
            } catch (e) {
              console.warn(`[sqlite-vec-index] enrichment failed for chunk ${chunk.id}:`, e);
            }
          }
        ...
        this.runInTransaction(() => {
          ...
          for (const chunk of chunks) {
            ...
            stmtInsertChunk.run(
              ...
              chunk.enrichment ? JSON.stringify(chunk.enrichment) : null,
              chunk.norm
            );
            for (const [term] of chunk.terms) {
