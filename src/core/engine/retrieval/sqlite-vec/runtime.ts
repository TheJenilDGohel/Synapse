...
export function ensureSqliteVecTable(service: SqliteVecServiceLike): void {
  ...
  try {
    ...
    service.db.exec('CREATE INDEX IF NOT EXISTS vec_chunks_embedding_idx ON vec_chunks(embedding) USING ivf8');
