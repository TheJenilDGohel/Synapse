...
export class VectorIndexService {
  ...
  getStatus(): Record<string, unknown> {
    ...
    const isColdStart = !this.data!.updated_at && (this.data!.total_files || 0) === 0 && (this.data!.total_chunks || 0) === 0;
    const state = isColdStart
      ? 'cold_start'
      : (this.data!.total_files || 0) > 0
        ? 'ready'
        : 'empty';
    return {
      backend: 'json',
      ...
      status_message: isColdStart
        ? 'Index has not been built yet'
        : state === 'empty'
          ? 'Index exists but currently has no indexed files'
          : 'Index ready',
