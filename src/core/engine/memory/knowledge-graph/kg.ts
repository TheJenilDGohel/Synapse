...
import { nowIso, cleanString, stableJson } from '../utils/index.js';
import type {
  ...
  KgEntityWithRelations, KgStats, DeleteEntityResult, ListEntitiesOpts, ListEntitiesResult, KgEntityListItem
} from '../types/index.js';
...
export async function queryEntityRelationships(
  ...
  const entityExists = await adapter.get<{ id: string }>(
  ...
  return {
    entity_id: id,
    entity_found: Boolean(entityExists),
    direction: dir,
    count: results.length,
    ...(entityExists ? {} : { suggestion: 'Entity not found. Use synapse_kg_list_entities to discover valid entity IDs.' }),
    triples: results
  };
}
...
export async function listEntities(
  adapter: Adapter,
  { type, nameContains, limit = 20, offset = 0 }: ListEntitiesOpts = {}
): Promise<ListEntitiesResult> {
  const safeLimit = Math.max(1, Math.min(Number(limit) || 20, 200));
  const safeOffset = Math.max(0, Number(offset) || 0);
  const typeFilter = cleanString(type, 100) || null;
  const nameFilter = cleanString(nameContains, 200);
  ...
  const totalRow = await adapter.get<{ count: number }>(
    `SELECT COUNT(*) AS count FROM kg_entities ${whereClause}`,
    params
  );
  const items = await adapter.all<KgEntityListItem>(
    `SELECT id, name, entity_type, memory_id, created_at, updated_at
       FROM kg_entities
       ${whereClause}
  ...
  const hasMore = safeOffset + count < total;
  return {
    total_count: total,
    count,
    limit: safeLimit,
    offset: safeOffset,
    has_more: hasMore,
    next_offset: hasMore ? safeOffset + count : null,
...
export async function queryTriplesAsOf(
  ...
  if (!Number.isFinite(parsed)) {
    throw new Error(`Invalid as_of date: ${asOfDate}`);
  }
  ...
  if (mode === 'transaction') {
    ...
    triples = await adapter.all<KgTripleWithNames>(
      `SELECT t.id, t.subject_id, t.predicate, t.object_id, t.valid_from, t.valid_to,
              t.confidence, t.source_memory_id, t.source_type, t.created_at,
              COALESCE(NULLIF(t.recorded_at, ''), t.created_at) AS recorded_at,
              s.name AS subject_name, o.name AS object_name
         FROM kg_triples t
         JOIN kg_entities s ON s.id = t.subject_id
         JOIN kg_entities o ON o.id = t.object_id
        WHERE (t.subject_id = ? OR t.object_id = ?)
          AND COALESCE(NULLIF(t.recorded_at, ''), t.created_at) <= ?
        ORDER BY COALESCE(NULLIF(t.recorded_at, ''), t.created_at) ASC`,
      [id, id, resolvedDate]
    );
  } else {
...
export async function getEntityTimeline(
  ...
  const triples = await adapter.all<KgTripleWithNames>(
    `SELECT t.id, t.subject_id, t.predicate, t.object_id, t.valid_from, t.valid_to,
       ...
       JOIN kg_entities o ON o.id = t.object_id
      WHERE t.subject_id = ? OR t.object_id = ?
      ORDER BY t.valid_from ASC, COALESCE(NULLIF(t.recorded_at, ''), t.created_at) ASC`,
    [id, id]
  );
