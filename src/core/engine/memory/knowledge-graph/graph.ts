import { cleanString, clampInt } from '../utils/index.js';
import type { Adapter, TraverseGraphOpts, TraverseGraphResult, DiscoverBridgesOpts, DiscoverBridgesResult, BridgeEntry } from '../types/index.js';
...
export async function traverseGraph(
  adapter: Adapter,
  { startEntityId, maxHops, direction, limit, entityType, predicates, maxPerDepth }: TraverseGraphOpts
  ...
  const totalLimit = clampInt(limit, 20, 1, 200);
  const perDepth = maxPerDepth === undefined || maxPerDepth === null ? null : clampInt(maxPerDepth, 10, 1, 100);
  const typeFilter = cleanString(entityType, 100) || null;
  const predicateFilters = Array.isArray(predicates)
    ? predicates.map((value) => cleanString(String(value || ''), 200)).filter(Boolean)
    : [];
  const predicateClause = predicateFilters.length > 0
    ? `AND t.predicate IN (${predicateFilters.map(() => '?').join(', ')})`
    : '';

  let sql: string;
  const params: unknown[] = [start, start, hops, ...predicateFilters, start];
  ...
  const filteredRows = rows.filter((row) => !typeFilter || row.entity_type === typeFilter);
  const limitedRows = perDepth
    ? Array.from(filteredRows.reduce((acc, row) => {
      const bucket = acc.get(row.depth) || [];
      if (bucket.length < perDepth) bucket.push(row);
      acc.set(row.depth, bucket);
      return acc;
    }, new Map<number, TraversalRow[]>()).values()).flat()
    : filteredRows;
  const finalRows = limitedRows.slice(0, totalLimit);

  return {
    start_entity_id: start,
    max_hops: hops,
    direction: dir,
    discovered_count: finalRows.length,
    limit: totalLimit,
    max_per_depth: perDepth,
    entity_type_filter: typeFilter,
    predicate_filters: predicateFilters,
    entities: finalRows.map(row => ({
