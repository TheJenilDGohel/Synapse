...
import { agentPrime as agentPrimeFn, type AgentPrimeInput, type AgentPrimeResult } from './agent-prime.js';
import { teach as teachFn, type TeachInput, type TeachResult } from './teach.js';
...
export class MemoryWorkflowService {
  ...
  private vectorIndex: { getStatus(): Record<string, unknown> } | null;

  constructor({ memory, getRuntimeSummary = null, search = null, vectorIndex = null }: WorkflowConfig) {
    this.memory = memory;
  ...
  async captureOutcome(input: Record<string, unknown> = {}): Promise<Record<string, unknown>> {
    ...
    const hasExplicitContent = Boolean(task || summary || details);
    const finalTask = task || 'Session completed';
    const finalSummary = summary || (hasExplicitContent
      ? ''
      : `Session completed with ${memorySummary.total_entries} memories and ${memorySummary.total_events} recorded events.`);
    const finalDetails = details || fallbackContent || (hasExplicitContent
      ? ''
      : 'No explicit task summary was provided. Captured as a session-end marker.');

    const captureInput = {
      event_type: eventType,
      status: defaultEventStatus(eventType, input.status),
      title: finalTask || finalSummary || finalDetails,
  ...
  async agentPrime(input: Record<string, unknown>): Promise<AgentPrimeResult> {
    return agentPrimeFn(
      { memory: this.memory, search: this.search, vectorIndex: this.vectorIndex },
